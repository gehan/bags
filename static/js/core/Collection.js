...
Collection = new Class({
  ...
  add: function(model, options) {
    ...
    if (!(this.model != null)) throw new Error("Model not defined for collection");
    if (typeOf(model) === 'array') {
      ...
      this._add(model);
      if (!options.silent) return this.fireEvent('add', [model]);
    } else {
  ...
  reset: function(models, options) {
    ...
    if (!options.silent) return this.fireEvent('reset', [this]);
  },
  _remove: function(model, options) {
    this.erase(model);
    if (!options.silent) return this.fireEvent('remove', [model]);
  },
